	...
	<?php foreach($topics as $key => $topic) : ?>
		...
		<?php 
			...
			if(isset($topic['first_postid']) && $topic['first_postid'] != 0){
				...
				$intro_posts = $wpforo->post->options['layout_extended_intro_posts_count']; if( $intro_posts < 1 ){ $intro_posts = NULL; } else { $intro_posts = ($intro_posts > 1) ? ($intro_posts - 1) : $intro_posts = 0; }
			...
			$post_toglle = $wpforo->post->options['layout_extended_intro_posts_toggle'];
