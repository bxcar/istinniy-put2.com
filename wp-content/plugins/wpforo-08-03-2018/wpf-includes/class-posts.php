...
class wpForoPost{
		...
		if( !$guestposting ){
			...
			if( !($this->wpforo->perm->forum_can('er', $post['forumid']) || 
					($this->wpforo->current_userid == $post['userid'] && $this->wpforo->perm->forum_can('eor', $post['forumid']) && 
						$diff < $this->wpforo->post->options['eor_durr'])) ){
	...
	function delete($postid, $delete_cache = true){
		...
		$diff = current_time( 'timestamp', 1 ) - strtotime($post['created']);
		if( !($this->wpforo->perm->forum_can('dr', $post['forumid']) || ($this->wpforo->current_userid == $post['userid'] && $this->wpforo->perm->forum_can('dor', $post['forumid']) && $diff < $this->wpforo->post->options['dor_durr'])) ){
	...
	*/
	function get_post_url( $arg, $absolute = true ){
		...
		if( is_array($post) && !empty($post) && $postid ){
			...
			if( $post['topicid'] ){
				if( !$position ) $position = $this->wpforo->db->get_var("SELECT COUNT(`postid`) FROM `".$this->wpforo->db->prefix."wpforo_posts` WHERE `topicid` = ".intval($post['topicid'])." AND `postid` <= " . ($post['parentid'] ? intval($post['parentid']) : intval($postid) ) );
				if( $position <= $this->wpforo->post->options['posts_per_page'] ) return wpforo_home_url($url, false, $absolute ) . "#post-" . intval($postid);
				if( $position && $this->wpforo->post->options['posts_per_page'] ) {
                    $paged = ceil($position / $this->wpforo->post->options['posts_per_page']);
                }
                else{
                    $paged = 1;
                }
	...
	*/
    ...
    function is_approved( $postid ){
        $post = $this->wpforo->db->get_var( "SELECT `status` FROM " . $this->wpforo->db->prefix."wpforo_posts WHERE `postid` = " . intval($postid) );
