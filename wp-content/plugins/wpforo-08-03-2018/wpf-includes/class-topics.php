...
class wpForoTopic{
	...
	public function edit( $args = array() ){
		...
		if( !($this->wpforo->perm->forum_can('et', $topic['forumid']) || ($this->wpforo->current_userid == $topic['userid'] && $this->wpforo->perm->forum_can('eot', $topic['forumid']) && $diff < $this->wpforo->post->options['eot_durr'])) ){
	...
	function delete($topicid = 0, $delete_cache = true ){
		...
		$diff = current_time( 'timestamp', 1 ) - strtotime($topic['created']);
		if( !($this->wpforo->perm->forum_can('dt', $topic['forumid']) || ($this->wpforo->current_userid == $topic['userid'] && $this->wpforo->perm->forum_can('dot', $topic['forumid']) && $diff < $this->wpforo->post->options['dot_durr'])) ){
		...
		if(!empty($args)){
			...
			if( isset($topic['status']) && $topic['status'] && !wpforo_is_owner($topic['userid'])){
				if( isset($topic['forumid']) && $topic['forumid'] && !$this->wpforo->perm->forum_can('au', $topic['forumid']) ){
				    $this->wpforo->current_object['status'] = 'unapproved';
					return array();
				}
			}
