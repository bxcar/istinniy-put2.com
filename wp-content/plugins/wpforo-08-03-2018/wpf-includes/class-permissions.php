...
class wpForoPermissions{
	...
	*/
 	...
 	function usergroup_cans_form( $groupid = FALSE ){
		...
		$can_data = array();
		$cans = $this->wpforo->usergroup->cans;
	...
	*/
	function add( $title, $cans = array(), $key = '' ){
		$cans = wpforo_parse_args($cans, array_map('wpforo_return_zero', $this->wpforo->forum->cans));
		if(!$key) $key = $title;
	...
	function usergroups_can( $do ){
		$usergroupids = array();
		$usergroups = $this->wpforo->usergroup->get_usergroups();
