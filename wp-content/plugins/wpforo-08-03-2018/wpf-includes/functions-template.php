...
function wpforo_login_url(){
	global $wpforo;
	if(isset($wpforo->member->options['login_url']) && $wpforo->member->options['login_url']){
...
function wpforo_register_url(){
	global $wpforo;
	if(isset($wpforo->member->options['register_url']) && $wpforo->member->options['register_url']){
...
function wpforo_lostpass_url(){
	global $wpforo;
	if(isset($wpforo->member->options['lost_password_url']) && $wpforo->member->options['lost_password_url']){
...
class wpForo_Widget_login_form extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		<?php if( is_user_logged_in() && !empty($wpforo->current_user) ) : ?>
			...
			<div class="h-header">
	      	<?php if( $wpforo->perm->usergroup_can('va') && wpforo_feature('avatars', $wpforo) ): $rsz =''; ?>
...
function wpforo_member_title( $member = array(), $echo = true, $usergroup = true ){
	...
	$rating_title_ug_enabled = ( isset($wpforo->member->options['rating_title_ug'][$member['groupid']]) && $wpforo->member->options['rating_title_ug'][$member['groupid']] ) ? true : false ;
	$usergroup_title_ug_enabled = ( isset($wpforo->member->options['title_usergroup'][$member['groupid']]) && $wpforo->member->options['title_usergroup'][$member['groupid']] ) ? true : false ;
...
function wpforo_member_badge( $member = array(), $sep = '', $type = 'full' ){
	global $wpforo;
	$rating_badge_ug_enabled = ( isset($wpforo->member->options['rating_badge_ug'][$member['groupid']]) && $wpforo->member->options['rating_badge_ug'][$member['groupid']] ) ? true : false ;
...
function wpforo_fields( $fields, $echo = true ){
	...
	$html = $wpforo->tpl->form_fields( $fields );
	if( $echo ){
		echo $html;
	}
	else{
...
function wpforo_register_page_field_values( $fields ){
	global $wpforo;
	$wpforo->form['value']['user_login'] = (isset($_POST['wpfreg']['user_login'])) ? sanitize_user($_POST['wpfreg']['user_login']) : '';
	$wpforo->form['value']['user_email'] = (isset($_POST['wpfreg']['user_email'])) ? sanitize_email($_POST['wpfreg']['user_email']) : '';
...
function wpforo_search_page_field_values( $fields ){
    global $wpforo;
    $wpforo->form['value'] = ( !empty($_GET) ? (array) $_GET : array() );
    $wpforo->form['varname'] = '';
}
...
function wpforo_user_avatar( $user, $size, $attr = '', $lastmod = false ){
	...
	elseif( is_array($user) && !empty($user) ){
		$avatar_html = $wpforo->member->avatar($user, $attr, $size); //'alt="'.esc_html($display_name).'"'
	}
...
function wpforo_search_fields(){
    ...
    if( $wpforo->member->options['search_type'] == 'search' ){
        $fields = array(
            array(
                array(
                    array(
