...
(function($) {	
	...
	var wcff = function() {
		...
		this.registerEvents = function() {			
			...
			$(document).on( "click", "a.field-remove-rule", function(e) {
			...
			$(document).on( "click", ".wcff-meta-row label.wcff-field-label", this, function() {
				if( !$(this).closest( ".wcff-meta-row" ).is(".active") ){
					$(this).closest( ".wcff-meta-row" ).find("a.wcff-field-edit").trigger("click");
				}
			});			
			...
			$(document).on( "click", "a.wcff-field-edit", this, function(e) {
				...
				if( !$( e.target ).is( "input" ) ){
					if( $("#wcff-field-type-meta-label").length != 0 && $("#wcff-field-type-meta-label").val().trim == "" ){
						...
						$("#wcff-field-type-meta-label-temp").parent().html( $("#wcff-field-type-meta-label-temp").val() );
						$( ".wcff-meta-option .wcff-cancel-update-field-btn" ).hide();
						$( ".wcff-meta-option .wcff-field-edit" ).show();
						$(".wcff-meta-row").removeClass("active");
						$(this).closest( ".wcff-meta-row" ).addClass("active");
						mask.doMask( $(this).parent().parent().parent().parent().parent() );
						e.data.prepareRequest( "GET", "wcff_fields", { field_key : $(this).closest(".wcff-meta-row").find( ".wcff-field-edit" ).attr("data-key") } );
						e.data.dock( "wcff_fields", $(this) );		
						targetArr.find( ".wcff-field-edit" ).hide();
						targetArr.find( ".wcff-cancel-update-field-btn" ).show();
						targetArr.find( ".wcff-field-label" ).html( '<input type="text" id="wcff-field-type-meta-label-temp" value="'+targetArr.find( ".wcff-field-label" ).text()+'">' );
						$( ".wcff_fields_factory_header" ).hide();
					}
				}
				e.preventDefault();
			});			
			...
			$(document).on( "click", "#wcff-factory-tab-header > a:not(.wcff-add-new-field)", function(e) {
				$("#wcff-factory-tab-container > div").fadeOut("fast");
				$("#wcff-factory-tab-header > a:not(.wcff-add-new-field)").removeClass();
				$(this).addClass("selected");			
				$($(this).attr("href")).fadeIn("fast");
				e.preventDefault();
			});
			$(document).on( "click", ".wcff-pricing-rule-toggle > a, .wcff-fee-rule-toggle > a, .wcff-pricing-type-of-price-toggle > a, .wcff-fee-type-of-fee-toggle > a, .wcff-field-type-of-field-toggle > a", function(e) {
				$(this).parent().find("a").removeClass("selected");
				$(this).addClass("selected");
				e.preventDefault();
				if( $(this).parent().is( ".wcff-pricing-type-of-price-toggle" ) ){
					$(this).closest( "tr" ).find( ".wcff-pricing-rules-amount" ).attr( "placeholder", $(this).attr( "data-tprice" ) == "cost" ? "Amount" : "Percentage" );
				}
				if( $(this).parent().is( ".wcff-fee-type-of-fee-toggle" ) ){
					$(this).closest( "tr" ).find( ".wcff-fee-rules-amount" ).attr( "placeholder", $(this).attr( "data-tprice" ) == "cost" ? "Amount" : "Percentage" );
				}
			});			
			...
			$(document).on( "click", ".wcff-cancel-update-field-btn", this, function(e) {				
				...
				/* Clear the field rules section */
				$("#wcff-factory-field-rules-wrapper").children().not("#wcff-add-field-rule-btn").remove();
				...
				$("#wcff-factory-tab-header a[href='#wcff-factory-tab-pricing-rules'], #wcff-factory-tab-header a[href='#wcff-factory-tab-fields-rules']").hide();
			...
			$(document).on( "change", "#wcff-field-type-meta-type", this, function(e) {			
				$( "#wcff-field-type-meta-type-label" ).html( '<img style="margin-bottom: 2px;" src="'+wcff_var.plugin_dir+'/assets/img/'+ $( this ).val() +'.png" alt="Field Icon"><label style="margin-left: 6px;vertical-align: super;">'+  $( this ).val() +'<label>' );
				
			});		
			
			$(document).on( "keyup", "#wcff-field-type-meta-label-temp", this, function(e){
				$( "#wcff-field-type-meta-label" ).val( $(this).val() );
				$( "#wcff-field-type-meta-name" ).val( e.data.sanitizeStr( $(this).val() ) );
		...
		this.addPricingRule = function( target ) {
			var html = '',
				targetAttr = target.is("#wcff-add-field-rule-btn") ? "field" : "pricing";
		...
		this.renderSingleView = function( _target ) {
			...
			$("table.wcff_field_rules_table").remove();
			...
			$( "#wcff-field-type-meta-type-label" ).html( '<img style="margin-bottom: 2px;" src="'+wcff_var.plugin_dir+'/assets/img/'+this.activeField["type"]+'.png" alt="Field Icon"><label style="margin-left: 6px;vertical-align: super;">'+this.unEscapeQuote( this.activeField["type"] )+'<label>' );
			...
			if (wcff_var.post_type === "wccpf") {			
				...
				if( Object.prototype.toString.call( field_rules ) === '[object Array]' ) {
				    for(i = 0; i < field_rules.length; i++) {				    	
				    	this.renderPricingRow("field", field_rules[i], $("#wcff-add-field-rule-btn"));
				    }			
	    			var field_rules_count = $("#wcff-factory-field-rules-wrapper .wcff-field-row");
			...
			if ( this.activeField["type"] !== "email" && this.activeField["type"] !== "label" && this.activeField["type"] !== "hidden") {
				$("#wcff-factory-tab-header a[href='#wcff-factory-tab-pricing-rules'], #wcff-factory-tab-header a[href='#wcff-factory-tab-fields-rules']").show();
			} else {
			...
			// move field metas to perticular field.
			_target.closest( ".wcff-meta-row" ).after( $("#wcff_factory").detach() );
			$("#wcff_factory").show();
		};
		...
		this.onFieldSubmit = function( target ) {
			...
			if( payload.label !== "" ) {
				...
				if( $("#wcff_fields_factory").attr("action") === "POST" ) {
					...
					if( this.activeField["key"] == undefined ||  this.activeField["key"] == "" ){
						return false;
					}
					...
					$("table.wcff-field-row").each(function() {				
						me.fetchPricingRules($(this), "field");				
					});
					...
					if( this.fieldRules.length > 0 ) {
						payload["field_rules"] = this.fieldRules;
					}
				}				
				...
				if(  $("#wcff_fields_factory").attr("action") == "PUT" ){
					$("#wcff-field-type-meta-label-temp").parent().html( $("#wcff-field-type-meta-label-temp").val() );
					$( "#wcff_fields" ).after( $("#wcff_factory").detach() );
					$("#wcff_factory").hide();
				}
			
			} else {
				$("#wcff-field-type-meta-label").addClass("wcff-form-invalid");
				$("#wcff-field-type-meta-label-temp").addClass("wcff-form-invalid");
			}			
		};
		...
		this.buildPricingWidgetDatePicker = function(_type, _val) {
			...
			if (_type == "pricing") {
				...
				target = $("#wcff-add-field-rule-btn");
			}		
			...
			if (_type === "pricing" || _type === "field") {
				...
				if( _type === "pricing" ){
					...
					for( var i = 0; i < field_lists.length; i++ ){
						...
						html += '<tr>';
						html += '<td>';
						html += '<label>'+ $(field_lists[i]).find(".wcff-field-label").text() +'<label>';
						html += '</td>';
						html += '<td>';
						html += '<div class="wcff-field-type-of-field-toggle"><a href="#" data-field_label="'+ fieldLabel +'" data-vfield="show" title="Show Field" class="field-show">Show</a><a href="#" data-vfield="hide" data-field_label="'+ fieldLabel +'" title="Hide Field" class="field-hide">Hide</a><a href="#" data-vfield="Nill" data-field_label="'+ fieldLabel +'" title="No rule" class="field-nill-rule selected">Nill</a></div>';
						html += '</td>';
						html += '</tr>';
					}
					html += '</tbody></table>';
				...
				html += '<div class="wcff-pricing-type-of-price-toggle">';
				...
				html += '<div class="wcff-fee-type-of-fee-toggle">';
			...
			if( _action === "product" ||
				...
				if( this.request.request === "GET" ) {	
					...
					if(this.response.status ) {
						...
						$("#wcff-empty-field-wcff-field-value-tdset").hide();
						$("#wcff-field-factory-footer").hide();
						$("#wcff_factory").hide();
						$(".wcff-add-new-field").html("+ Add Field");
						$("#wcff_fields_factory").attr("action","POST");	
			
						/* Hide prcing rule tab, Hide field rule tab */
						$("#wcff-factory-tab-header a[href='#wcff-factory-tab-pricing-rules'], #wcff-factory-tab-header a[href='#wcff-factory-tab-fields-rules']").hide();
	...
	$(document).ready( function() {
		$('#wcff-fields-set').sortable({
			update : function(){
				...
				$('.wcff-meta-row').each(function(){
					if( !$(this).is( "#wcff_factory" ) ){
						$(this).find("input.wcff-field-order-index").val(order);
						$(this).find("span.wcff-field-order-number").text((order+1));
						order++;
					}
				});
			}
		...
		function drop(e){
			  ...
			  if( $( "#wcff_fields_factory" ).attr( "action" ) == "PUT" ){
				  alert( "Please update previous changes.!" );
			  } else {
				  $( "#wcff-add-field-placeholder" ).after( $("#wcff_factory").detach() );
				  $("#wcff_factory").show();
				  $( "#wcff-field-type-meta-type" ).val( $( currenctDraggedField ).attr( "value" ).trim() );
				  $( "#wcff-field-type-meta-type" ).trigger( "change" );
			  }
			  return false;
		}
